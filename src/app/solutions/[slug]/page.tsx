...
export async function generateMetadata({
    ...
    const title = category?.name || (posts[0]?.title as string | undefined) || "Giải pháp";
    const description =
...
export default async function SolutionCategoryPage({ params }: SolutionCategoryPageProps) {
    ...
    const categoryName = category?.name || "Giải pháp";
            ...
            >
                ...
                <div className="relative max-w-7xl mx-auto px-6 text-center">
                    <nav className="flex items-center justify-center gap-2 text-sm text-gray-300 mb-4">
                        ...
                        <Link href={categoryLink || "/solutions"} className="hover:text-white transition-colors">
                            Giải pháp
                        </Link>
                        <BreadcrumbArrow className="w-4 h-4 text-gray-400" />
                ...
                <div className="max-w-7xl mx-auto px-6 -mt-20 pb-16">
                    ...
                    {posts.length > 0 ? (
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
                            {posts.map((post) => {
                                const href = `${categoryLink || "/solutions"}/${post.slug || ""}`;
