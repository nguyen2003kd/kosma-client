...
export default function CategoryTab({
  ...
  const renderItem = (category: Category) => {
    ...
    return (
      ...
      >
        <div
          className={`w-9 h-9 md:w-11 md:h-11 rounded-lg md:rounded-xl flex items-center justify-center mb-2 md:mb-2.5 ${isActive ? "bg-blue-600" : "bg-blue-100 group-hover:bg-blue-200"
            }`}
        >
          <DynamicIcon
            name={category.icon_url || "LayoutGrid"}
            className={`w-4.5 h-4.5 md:w-5.5 md:h-5.5 ${isActive ? "text-white" : "text-blue-600"
              }`}
          />
        </div>
        <span
          className={`text-[11px] md:text-[13px] font-semibold text-center leading-tight ${isActive ? "text-blue-600" : "text-gray-700"
            }`}
        >
          {category.name}
        </span>
      ...
      >
        <CarouselContent className="-ml-2 md:-ml-4">
          ...
          <CarouselItem className="pl-2 md:pl-4 basis-auto">
            ...
            >
              <div
                className={`w-9 h-9 md:w-11 md:h-11 rounded-lg md:rounded-xl flex items-center justify-center mb-2 md:mb-2.5 ${!selectedCategory ? "bg-blue-600" : "bg-blue-100 group-hover:bg-blue-200"
                  }`}
              >
                <LayoutGrid
                  className={`w-4.5 h-4.5 md:w-5.5 md:h-5.5 ${!selectedCategory ? "text-white" : "text-blue-600"
                    }`}
                />
              </div>
              <span
                className={`text-[11px] md:text-[13px] font-semibold text-center ${!selectedCategory ? "text-blue-600" : "text-gray-700"
                  }`}
              >
                {t("allCategories")}
              </span>
