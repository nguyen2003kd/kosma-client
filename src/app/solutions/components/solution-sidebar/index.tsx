...
export default function SolutionSidebar({
  ...
  return (
    <div className="sticky top-6 space-y-4">
      {/* Latest Solutions Card */}
      <Card className="overflow-hidden">
        {/* Header */}
        <div className="bg-[#1e40af] px-4 py-3 flex items-center justify-between">
          <h3 className="text-base font-bold text-white">{t("featuredSolutions")}</h3>
          <Newspaper className="w-5 h-5 text-white" />
        </div>

        {/* Content - Compact List */}
        <div>
          {relatedSolutions.slice(0, 5).map((solution, index) => (
            <a
              key={solution.id}
              href={`/solutions/${solution.slug || ""}`}
              className={`flex gap-4 group hover:bg-blue-50 px-4 py-3 transition-colors ${index !== relatedSolutions.slice(0, 5).length - 1
