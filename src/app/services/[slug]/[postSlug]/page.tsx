...
interface ServicePostDetailPageProps {
...
async function getRelatedServices(excludeId?: string): Promise<PostExtended[]> {
  try {
...
async function getLatestServices(excludeId?: string): Promise<PostExtended[]> {
  try {
    const data = await getApiV10Post({
      filters: "is_hidden==false , is_service==true",
      sortField: "created_at",
...
export async function generateMetadata({
  params,
}: ServicePostDetailPageProps): Promise<Metadata> {
...
export default async function ServicePostDetailPage({ params }: ServicePostDetailPageProps) {
  const { slug, postSlug } = await params;
  const post = await getPost(postSlug);
  ...
  const categoryName = await getCategoryName(categoryLink);
